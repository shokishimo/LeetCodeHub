...
class Solution {
    public boolean exist(char[][] board, String word) {
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[i].length; j++) {
                if (recursiveSerach(board, word, 0, i, j))
                    return true;
            }
        }
        return false;
    ...
    private boolean recursiveSerach(char[][] board, String word, int ind, int i, int j) {
        if (ind >= word.length()) return true;
        ...
        board[i][j] += 100;
        boolean res  = recursiveSerach(board, word, ind+1, i-1, j)
                || recursiveSerach(board, word, ind+1, i+1, j)
                || recursiveSerach(board, word, ind+1, i, j+1)
                || recursiveSerach(board, word, ind+1, i, j-1);
        board[i][j] -= 100;
        return res;
    }
